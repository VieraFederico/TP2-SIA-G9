...
from genetics_algorithm.fitness.pixel_difference.mae import PixelDifferenceFitnessMAE, _mae
from genetics_algorithm.fitness.pixel_difference.mse import _mse
from genetics_algorithm.fitness.pixel_difference.ssim import _ssim
...
class PixelDifferenceFitnessTest(unittest.TestCase):
    ...
    def test_evaluate_run(self):
        ...
        ind = Individual(target_image.width, target_image.height, 20)
        fitness_fn = PixelDifferenceFitnessMAE(target_image)
        result = fitness_fn.evaluate(ind)
